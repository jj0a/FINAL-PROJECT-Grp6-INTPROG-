...
include('connection_file/dbh.inc.php'); 
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) { 
    ...
    // Database connection should be established here (assuming $conn is defined and connected)
    if (!$conn) {
        ...
        if ($stmt) {
            ...
            if ($result->num_rows > 0) {
                ...
                if (password_verify($password, $row['password'])) { // Verify hashed password
                    ...
                    if ($rememberMe) {
                        setcookie("username", $username, time() + (7 * 24 * 60 * 60), "/");  // Store for 7 days
                    } else {
                    ...
                    $loginNotify = "Incorrect password.";
                }
            } else {
                // Notify user if no account exists with the provided email
                $loginNotify = "Account doesn’t exist. Please register.";
            }
            $stmt->close();
        } else {
            $loginNotify = "Database error: failed to prepare statement.";
        }
    } else {
        $loginNotify = "Please fix the validation errors.";
    }

    //Validation and insertion process of submitting a comment
    } elseif (isset($_POST['submitComment'])) { // Comment Form

       // $name = trim($_POST['name']);
       // $message = trim($_POST['message']);
       $name = filter_var(trim($_POST['name']), FILTER_SANITIZE_STRING);  //Using Filter
       $message = filter_var(trim($_POST['message']), FILTER_SANITIZE_STRING); //Using Filter
    ...
    // Function to display username from session or cookie if available
    function displayUsername($sessionKey, $cookieKey) {
        if (isset($_SESSION[$sessionKey]) && !empty($_SESSION[$sessionKey])) {
            ...
            echo htmlspecialchars($_COOKIE[$cookieKey]);
        } else {
...
<a href="#" class="title">
   ...
   <!--For menu icon style and will transform to X-->
    <div class="hamburger">
        <div class="bar"></div>
        <div class="bar"></div>
        <div class="bar"></div>
    ...
    <div class="form">
        ...
        <form action="ideaProfile.php" method="POST">
            ...
            <div class="form-element">
                ...
                <span class="error"> <?php echo $loginNotify;?></span>
            </div>
            <div class="form-element">
...
<div class="popup-comment">
    ...
    <!--Our php include footer code starts here here-->
    <?php 
